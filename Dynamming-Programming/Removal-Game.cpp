...
//---------------------------------
int solve(vector<int> a,int n,int f,int i,int j){
    if(i>j) return 0;
    if(i==j) return a[i];
    if(j=i+1) return max(a[i],a[j]);
    if(f==1) return max(solve(a,n,1-f,i+1,j)+a[i],solve(a,n,1-f,i,j-1)+a[j]);
    else return min(solve(a,n,1-f,i+1,j)+a[i],solve(a,n,1-f,i,j-1)+a[j]);
}
void solve(){
    arr(a,n);
    cout<<solve(a,n,1,0,n-1)<<endl;
}
