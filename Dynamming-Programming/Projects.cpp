...
bool cm(pair<int,pair<int,int>> &a,pair<int,pair<int,int>> &b){
...
void solve(){
    ...
    sort(range(v),cm);
    vector<int> ans(n);
    for(int i=0;i<n;i++) ans[i]=v[i].second.second;

    for(int i=1;i<n;i++){
        for(int j=0;j<i;j++){
            if(v[i].first>v[j].second.first and ans[i]<ans[j]+v[i].second.second)
                ans[i]=ans[j]+v[i].second.second;
        }
    }
    cout<<*max_element(range(ans))<<endl;
}
